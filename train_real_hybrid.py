...
class ActionBlendWrapper(gym.Wrapper):
    """
    Intercepts RL actions and blends them with a Proportional controller 
    when the pendulum approaches the upright position.
    """
    def __init__(self, env, blend_outer_deg=20.0, blend_inner_deg=10.0, max_voltage=12.0):
        super().__init__(env)
        self.blend_outer = np.radians(blend_outer_deg)
        self.blend_inner = np.radians(blend_inner_deg)
        self.max_voltage = max_voltage
        self.last_voltage = 0.0
        self.current_obs = None

        # Proportional Gains
        self.K1 = float(-4.4681 * 0.25)
        self.K2 = float(-1.6210 * 0.50)
        self.K3 = float(-39.5037 * 0.75)
        self.K4 = float(-5.0402 * 0.60)
        self.K5 = float(-0.4387 * 0.50)
    ...
    def step(self, action):
        # 1. Decode observation (Assuming standard [cos, sin, cos, sin, vel, vel] format)
        obs = self.current_obs
        pend_pos = np.arctan2(obs[1], obs[0])
        arm_pos = np.arctan2(obs[3], obs[2])
        pend_vel = obs[4] * 15.0  # Reverse the velocity scaling
        arm_vel = obs[5] * 25.0

        abs_theta = abs(pend_pos)

        # 2. Compute Proportional Action (always computed)
        prop_u = -(self.K1 * arm_pos + self.K2 * arm_vel + 
                   self.K3 * pend_pos + self.K4 * pend_vel + self.K5 * self.last_voltage)
        prop_v = np.clip(prop_u, -self.max_voltage, self.max_voltage)

        # RL action mapped to voltage
        rl_v = float(action[0]) * self.max_voltage

        # 3. Weighted Action Blending
        if abs_theta <= self.blend_inner:
            # Inside 10 degrees: Pure Proportional Stabilization
            final_v = prop_v
        elif abs_theta < self.blend_outer:
            # Between 10 and 20 degrees: Smooth blend from RL to Proportional
            alpha = (abs_theta - self.blend_inner) / (self.blend_outer - self.blend_inner)
            final_v = alpha * rl_v + (1.0 - alpha) * prop_v
        else:
            # Outside 20 degrees: Pure RL Swing-up
            final_v = rl_v

        # Re-normalize the final voltage to [-1, 1] for the environment
        final_action = np.array([final_v / self.max_voltage], dtype=np.float32)
        self.last_voltage = final_v

        # 4. Step the underlying hardware environment with the BLENDED action
        next_obs, reward, terminated, truncated, info = self.env.step(final_action)
        self.current_obs = next_obs

        # Note: SB3 will still log the original RL `action` in its replay buffer, 
        # which acts as "residual learning" in the blend zone.
...
def main():
    ...
    p.add_argument("--save_dir", default="runs/hybrid_models")
    ...
    # Wrap the base environment with the Action Blend wrapper
    env = ActionBlendWrapper(
        env, 
        blend_outer_deg=20.0, 
        blend_inner_deg=10.0, 
        max_voltage=args.max_voltage
    )
